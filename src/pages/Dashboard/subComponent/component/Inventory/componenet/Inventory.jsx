...
import { EyeIcon, Trash2Icon, AlertCircle, Box, CheckCircle, XCircle, ArrowUp01, ArrowUp } from "lucide-react";
...
const Inventory = () => {
  ...
  const { products, isLoading, isError, totalProducts, productsCount, revenueIn7Days, notInStock, totalRevenue, lowstock, categoryIn7Days } = useGetProduct();
  ...
  const addProduct = () => {
    ...
    // console.log(showProductForm, 'showProductForm')
  };
  ...
  if (id) {
    ...
    return ""; // or handle the case when id is undefined/null
  }
};
  ...
  // const lowStockCount = product.filter((item) => item.stock > 0 && item.stock < 10).length;
  // const outOfStockCount = product.filter((item) => item.stock === 0).length;
  // const totalProducts = product.length;
  // const availableProducts = product.filter((item) => item.stock > 0).length;
  ...
  return (
    <div>
      <main className="p-2 bg-gray-100 dark:bg-gray-900 dark:text-white">
        <div className="container mx-auto">
          ...
          <div className="flex gap-4 flex-col sm:flex-row items-center justify-between pb-6">
            <div className="bg-white shadow-md rounded-lg p-3 w-56 h-36">
              <h2 className="text-gray-800 text-sm font-semibold dark:text-blue-500 mb-1">
                <Box className="h-6 w-6 text-blue-500 mr-2 inline" /> Categories:
              </h2>
            ...
            <div className="bg-white shadow-md rounded-lg p-3 w-56 h-36">
              <h2 className="text-sm font-semibold text-gray-800 dark:text-green-400 mb-1">
                <CheckCircle className="h-6 w-6 text-green-400 mr-2 inline" /> Total Products:
              </h2>
            ...
            <div className="bg-white shadow-md rounded-lg p-3 w-56 h-36">
              <h2 className="text-sm font-semibold text-gray-800 dark:text-purple-500 mb-1">
                <ArrowUp className="h-6 w-6 text-purple-500 mr-2 inline" /> Top Selling Products:
              </h2>
              <p className="text-gray-500 dark:text-gray-800 font-semibold text-sm">
                <span className="flex justify-between gap-4 items-center pt-6 px-2 ">
                  <p className="flex flex-col">
            ...
            <div className="bg-white shadow-md rounded-lg p-3 w-56 h-36">
              <h2 className="text-sm font-semibold text-gray-800 dark:text-red-500 mb-1">
                <XCircle className="h-6 w-6 text-red-500 mr-2 inline" /> Low Stock Products:
              </h2>
              <p className="text-gray-500 dark:text-gray-800 font-semibold text-sm">
                <span className="flex justify-between gap-4 items-center pt-6 px-2 ">
                  <p className="flex flex-col">
          ...
          <div className="flex justify-between mb-4 items-center space-y-2 sm:space-y-0 sm:space-x-4 flex-col sm:flex-row">
              ...
              <div className="flex space-x-4">
                ...
                <button className="bg-[#163b8d] hover:bg-[#2563eb] px-4 py-2 rounded-md text-white font-semibold tracking-wide cursor-pointer">
                  Filter
                </button>
              </div>
            </div>
                    ...
                    <tr key={prod.id}>
                      <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                        <div className="flex items-center">
                          <div className="ml-3">
                            <p className="text-gray-900 whitespace-no-wrap">{slice(prod._id, 6)}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                        <p className="text-gray-900 whitespace-no-wrap">{prod.name}</p>
                      </td>
                      <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                        <p className="text-gray-900 whitespace-no-wrap">{prod.cost}</p>
                      </td>
                      <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
          ...
          {/* <div className="flex justify-between mt-4">
            <button
              ...
              className={`bg-gray-200 text-gray-600 py-2 px-4 rounded-md ${
            ...
            <button
              ...
              className={`bg-gray-200 text-gray-600 py-2 px-4 rounded-md ${
                currentProducts.length < productsPerPage ? "cursor-not-allowed" : "hover:bg-gray-300"
              }`}
            >
              Next
            </button>
      ...
      {showProductForm && <AddProductModal type={'Add Product'} showProductForm={showProductForm} cancelButton={handleClose}  />}
      {showDeleteModal && (
